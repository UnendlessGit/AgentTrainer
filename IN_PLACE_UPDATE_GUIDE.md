...
Runtime predictions contain two different semantics. Keyboard and mouse-button outputs are held state and may remain active across action ticks inside `InputInjector`. Game Camera and scroll outputs are additive and are consumed only once for each new inference result; stale timer ticks cannot replay the last movement. Policy v9 does not feed any demonstrated or predicted action back into its Transformer, so runtime permissions and calibrated thresholds affect only the physical execution firewall rather than creating hidden model state. Game Camera always emits `.mouseMoved` independently of held buttons. Validation selects the smallest stable 0.5–4.5 pixel pre-rounding deadzone, stores it in the immutable report, and `InputInjector` passes that exact value back to `GameCameraContract.postedDelta`; older reports fall back to 1.5 pixels. The explicit tagged zero-delta event used during disable/stop remains intentional.

Policy v9 also has a freshness deadline. ScreenCaptureKit `idle` means the source did not change, so runtime reuses the last usable surface for a fresh perception and advances the bounded visual-memory ring with an unchanged frame rather than falsely tripping that deadline; blank/suspended/stopped frames are never reused. Once at least one prediction has been published, an action tick stops the complete runtime if MLX has not replaced it within `max(350 ms, 3 / Perception FPS)`. Stop blocks new action work, drains the action queue, releases held state, and drops visual memory before waiting for capture and inference drains. Concurrent Stop callers join one teardown and the UI remains Starting/Stopping until it completes. Runtime architecture family, exact vision, visual-memory layout, timing, channels, cursor visibility, and Auto mouse mode come from the selected immutable version; mutable profile or Record-tab edits may not change a running brain's contract. App and runtime startup are both revision-tokened and single-flight so Stop during lookup or weight loading cannot later install an orphan model or input injector.
...
10. Leave the target visually unchanged and confirm `idle` frames keep perception alive without repeated stale-inference shutdowns.
11. Run and stop the same AI repeatedly, including a double Run/Stop shortcut press during startup; confirm only one runtime exists, Stop remains visible until cleanup finishes, and MLX cache returns near its post-warm-up baseline.
