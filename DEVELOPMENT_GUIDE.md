...
This is the durable engineering reference for the stable AgentTrainer 2.6.6 release. Keep it focused on contracts that a future change must preserve; release history belongs in Git.
...
Training uses calibrated class/transition weighting, focal binary loss, dataset-level sparse continuous weighting, the v6 generalization pipeline, deterministic salience-balanced locality order, compiled MLX updates, bounded held-out evaluation, and adaptive cosine scheduling. Positive binary weights are applied both as static loss weights and as a loss-only `log(weight)` offset; this preserves rare-positive gradients without shifting saved raw logits toward runtime's `0.5` held threshold. Prediction-dependent focal weights affect only the numerator and are normalized by static class/transition weight, so gamma must not cancel on equally difficult examples. Relative-mouse and scroll activity use bounded training-split inverse-frequency weights so rare signed targets are not diluted by idle rows.

`TrainingObjectiveContract.schemaVersion` is 2 and participates in exact-checkpoint signatures independently of packed data and tensor shapes. Objective v2 always replaces a fixed share of past-control histories with exact zeros, independently of optional generalization, so a temporal policy must learn to initiate actions from visual evidence. `TrainingValidationContract` v4 evaluates held-out rows with causal visual context but zero past controls; teacher-forced action history may never select a best brain again. This is a stress test, not an all-or-nothing runtime certification: a supported head with zero executable true positives produces a persistent quality warning. Supervised versions record objective v2, active-row coverage, the per-head report, and the exact optimizer step that produced validation metrics. RL descendants preserve supervised ancestry rather than claiming a newer objective. Objective changes must increment the contract, retain compatible active brain weights as a warm start, restart optimizer state, and recalibrate validation. Do not change these semantics without focused startup-history, sparse-control, validation, and compatibility tests.
...
- Validation shares deterministic temporal work and head logits before gathering them back to every held-out label, forces the complete past-control tensor to zero, then derives loss and predictions from that one compiled forward graph.
...
Completed training publishes an immutable version. The first finite held-out candidate is always preserved. A checkpoint where every sufficiently supported discrete head crosses the execution threshold outranks one with a collapsed head; among equally responsive candidates, lower aggregate loss wins unless a supported sparse head regresses sharply. If no fully responsive candidate exists, completion still activates the best available brain and surfaces exact per-head warnings. Run treats legacy-objective, weak-head, sparse-data, and lossy-vision findings as diagnostics; only actual model-schema, tensor-layout, or control-contract incompatibilities are fatal. Periodic autosaves are bounded, never prune the active or protected brain, and identify the exact step at which carried validation metrics were measured.
