...
This is the durable engineering reference for the stable AgentTrainer 2.7.5 release. Keep it focused on contracts that a future change must preserve; release history belongs in Git.
...
- `Training/BinaryDecisionCalibration.swift` — per-output held-out threshold selection and fail-closed binary execution semantics
...
Dropout and `GeneralizationConfiguration` are stochastic training regularization, not part of the learned tensor layout. Editing only these values must preserve and reload a current-objective active v6 brain. Because the exact objective configuration changed, the checkpoint signature still includes them and begins a fresh optimizer sequence rather than claiming an invalid exact resume. The separate pre-v4 supervised migration rule below remains authoritative for older brains.
...
`GeneralizationConfiguration` owns five training-only defenses. Structured YUV luminance/contrast/chroma variation and small neutral rectangles alter pixels without moving absolute pointer geometry. Control-history dropout removes a selected output's complete trajectory, rare flips on exact learnable binary slots and bounded noise on exact continuous slots simulate imperfect prior outputs, and ignored duplicate modifier-key slots remain zero. Temporal-token dropout trains additional missing-history cases beyond the exact sentinel mask. Small binary label smoothing reduces brittle confidence while validation retains exact targets. Visual-embedding and fusion dropout occur only after unique visual work is gathered back to action rows, preserving an independent stochastic path per label. Inference applies none of these perturbations.

Training uses calibrated class/transition weighting, focal binary loss, dataset-level sparse continuous weighting, coverage-aware action-head weighting, the v6 generalization pipeline, deterministic salience-balanced locality order, compiled MLX updates, bounded held-out evaluation, and adaptive cosine scheduling. Positive binary weights are applied both as static loss weights and as a loss-only `log(weight)` offset; this preserves rare-positive gradients while leaving saved raw logits calibrated to demonstrated rates. Prediction-dependent focal weights affect only the numerator and are normalized by static class/transition weight, so gamma must not cancel on equally difficult examples. Relative-mouse and scroll activity use bounded training-split inverse-frequency weights so rare signed targets are not diluted by idle rows. Head reliability requires the larger of 16 active rows or the square root of the training-row count. A sufficiently demonstrated enabled head has weight 1 in the shared objective; an underrepresented head scales linearly down to a 0.1 floor so incidental data cannot consume the same encoder-gradient budget as reliable controls. Output-specific capability filtering remains authoritative when a keyboard/modifier output has no demonstration at all.

`TrainingObjectiveContract.schemaVersion` is 4 and participates in exact-checkpoint signatures independently of packed data and tensor shapes. Every policy's first complete objective-v4 epoch trains the ordinary fusion and action heads through the current image; temporal models receive an exact zero recurrent state, and a mid-epoch pause persists that curriculum state exactly. A compatible pre-v4 supervised version stays immutable and runnable, but its dataset-training successor starts from clean model weights. Real record-disjoint diagnostics over a 749-recording, 604,557-row cache showed that resetting only recurrent/fusion history was insufficient: the CNN still varied across images while the learned visual projection reduced embedding variance from roughly `3.8e-4` at initialization to `2.3e-9`, leaving action heads to emit marginal frequencies. Current-objective weight-only activations and reinforcement brains remain valid warm starts.

Before a temporal grounding epoch, the compact control projection and the first fusion layer's recurrent columns are zeroed. The bootstrap is deterministic: pixel augmentation, feature dropout, and label smoothing are disabled, and its effective learning rate is capped at `0.0003` so a large custom network cannot erase emerging visual structure. The objective preserves at least a modest `0.02` per-feature batch standard deviation in the visual embedding and adds a bounded pairwise positive-versus-negative logit-ranking term only for demonstrated controls represented by both classes in the current batch. Every hard negative participates directly instead of disappearing inside a class mean; calibrated BCE remains authoritative for probability learning. Later temporal epochs restore the user's configured rate, every regularizer, causal visual memory, and 80% zero-control-history exposure while retaining a half-weight current-image-only action objective. Current and auxiliary paths share one CNN/attention evaluation but receive independent post-encoder dropout. Immutable manifests persist whether grounding completed so a weights-only activation neither clears a mature recurrent path nor skips an interrupted bootstrap; RL descendants preserve the same flag.

`TrainingValidationContract` v5 evaluates held-out causal vision with zero past controls and calibrates discrete decisions. `BinaryDecisionContract` v1 stores one threshold at every action index, but adopts an executable threshold only with at least 5 positive and 20 negative examples, at least 5% recall, at least 50% precision, no more than 1% per-output false-positive rate, and Matthews correlation of at least 0.05. A safe boundary may replace an unsafe `0.5` default even with lower correlation; otherwise a safe default needs at least 0.01 MCC improvement before it moves. If neither default nor a candidate is safe, threshold `1` is an explicit disabled sentinel and must never execute even when a sigmoid rounds to exactly one. Validation, temporal feedback, input injection, and RL descendants consume the same vector and the same sentinel semantics. Reports persist supported and responsive per-output counts so aggregate head metrics cannot hide missing keys. Objective changes must restart optimizer state and recalibrate validation without rebuilding compatible packed data. Do not change these semantics without focused grounding, representation-collapse, ranking, threshold, runtime-consistency, sparse-control, validation, and compatibility tests.
...
- During the first visual-grounding epoch, the compiled GPU graph intentionally omits past-image encoding, control projection, recurrence, and stochastic regularization; the saved tensor layout is unchanged, the rate cap plus visual-variance/ranking safeguards are applied inside the same checkpointed optimizer update, and later epochs restore the ordinary temporal graph plus its persistent visual-only auxiliary after current-image fusion is established.
...
- Validation shares deterministic temporal work and head logits before gathering them back to every held-out label, forces the complete past-control tensor to zero, then derives loss, raw scores, per-output thresholds, and predictions from that one compiled forward graph.
...
Completed training publishes an immutable version. The first finite held-out candidate is always preserved. More individually responsive supported outputs outrank fewer; among equally responsive candidates, lower aggregate loss wins unless a supported sparse head or false-action rate regresses sharply. If no fully responsive candidate exists, the best available brain remains runnable with an exact incomplete-control warning. The checkpoint directory always retains the newest model, optimizer, scheduler, random state, and batch cursor for exact continuation. Periodic autosaves, pauses, and completed runs instead activate the best validated weights and the threshold vector measured for those exact weights, so a later regressed optimizer step cannot silently replace a better runner. Autosaves are bounded and never prune the active or protected brain.
...
At runtime, every successfully inferred perception stores its compact reduced-frame visual embedding with the sanitized controls predicted for that interval. Binary sanitization uses the immutable brain's per-output threshold vector before either temporal feedback or physical injection; a legacy or malformed vector falls back to `0.5`, while an explicit threshold of `1` is fail-closed and never executes. Exact spaced entries are selected from a fixed-capacity circular buffer. Unavailable startup slots use zero embeddings and controls; the first visual-grounding epoch establishes a current-image policy, its persistent auxiliary prevents later erasure, and objective-level zero histories reproduce runtime startup with causal visual context restored. Whole-token dropout covers additional missing-history cases. Sampling/appending remains O(1), and historical images are never re-encoded.
