...
- The app is local-only: no accounts, networking, telemetry, cloud training, or Python runtime.
- Native stack: Swift, SwiftUI, AppKit, ScreenCaptureKit, AVFoundation/VideoToolbox, Metal, and MLX Swift.
- Deployment floor: Apple silicon running macOS Sequoia 15.0. Builds use the current Tahoe SDK and remain compatible with macOS Tahoe 26. MLX Swift is pinned to the latest stable tag whose package manifest is accepted by Xcode 26.3, the final Xcode line available on Sequoia.
...
- Crystal V4 and Crystal V4 Fine-tuned + glass are user-protected profiles: automatic migration, autosave pruning, version deletion, learning reset, and profile deletion must leave both intact. Protection becomes sticky when either is saved, so later renames remain safe.
...
- `Preprocessing/VisionPreprocessor.swift`: Metal packing kernel and MLX packed-tensor expansion.
...
`events.atrevents` begins with `ATREVT01`, a UInt32 version, then 72-byte little-endian records. Keep old manifests decodable by using optional fields or explicit custom decoding when a required field is introduced. The cache key includes the complete recording manifests, preprocessing, rates, and history length; trims and recording exclusions therefore invalidate derived caches automatically.
...
3. `CaptureService` starts ScreenCaptureKit and hardware HEVC. The first complete frame's host-clock PTS becomes `hostStartNanos`; inputs before it are discarded. At that boundary, recording writes a zero-delta cursor snapshot before opening the input clock so every dataset has a correct initial absolute position. Legacy datasets prime position from their earliest pointer-bearing event, and mouse-button/scroll events also refresh position.
4. Stop disables and joins the event-tap run loop, then stops/drains ScreenCaptureKit and finalizes HEVC.
...
- 142–145 Shift, Control, Option, Command.
...
Before the first convolution, the policy appends normalized X and Y coordinate channels at the exact requested width and height. These channels retain absolute screen position after global spatial pooling and are part of the saved-weight architecture.

Live CNN inspection is presentation-only and does not change the learned-brain contract. `AgentPolicy.visualActivations` exposes post-ReLU spatial stages without adding modules or parameters. Activation Overlay compiles one lazy graph per selectable layer and reduces only the chosen stage; Feature Channels ranks on the GPU and transfers at most 16 spatial maps; Action Saliency computes positive Grad-CAM influence for a selected action head from the exact unsampled final convolution. Spatial copies are capped to a 96-pixel longest side. The normal compiled prediction function remains the only evaluated graph while inspection is disabled.
...
The collapsed technical details distinguish packed source values from the dense values processed by the network: chroma subsampling reduces packed cache/live-frame values, then MLX expands color to full-resolution Y/Cb/Cr; the policy appends two full-resolution coordinate channels; and the recurrent branch receives `max(1, historyLength) × 146` values. A zero history uses one all-zero row to preserve a valid recurrent shape. Bit depth changes quantization levels and meaningful source bits, not value count; resize policy changes framing; enabled control channels change loss/output behavior; architecture widths change parameters; and batch size repeats the complete per-decision input. Perception FPS determines the maximum new network-input rate. Action FPS determines the history time span and may maintain held state against a reused perception; transient relative-mouse and scroll slots are zero on action ticks without a new prediction. The displayed precision payload is nominal input-only storage and must not be described as total MLX or unified-memory use.

Model contract schema 3 contains both the fixed raw-delta scale and coordinate-aware vision input. Training-data schema 5 adds causal next-interval targets and trim-boundary priming; it invalidates only derived caches and exact-resume signatures, not runnable weights. On first launch at a new model schema, `WorkspaceStore.removeObsoleteModelArtifacts` preserves profiles and all recordings and removes only incompatible learned artifacts. Both protected AIs are explicitly skipped. Never broaden this migration to source recordings.
...
- Packed observation gathering writes directly into one pre-sized `Data` buffer with bulk `memcpy`; it never grows an append buffer row by row.
- Fixed-shape batch temporaries live inside an autorelease pool. MLX cache is bounded to 6% of physical memory with a 2 GiB ceiling; total MLX allocation is capped at 94% while retaining an OS reserve.
...
Exact checkpoint identity includes preprocessing, normalized channels, learning configuration/architecture, complete selected recording manifests, folder IDs, and output restrictions. Epoch count, maximum step budget, and autosave interval intentionally do not invalidate a checkpoint.
...
- Prediction state reuse is intentional when action FPS exceeds perception FPS, but only idempotent state may be reused: keyboard/button holds and absolute position remain available, while relative-mouse and scroll deltas are consumed once and recorded as zero on later stale ticks.
...
- Stop marks runtime stopped, cancels the action source, disables/joins safety input, stops and drains capture, drains action/inference queues, disables the injector, releases keys/buttons, associates the cursor, and posts a zero-delta move.
...
Do not reorder cleanup so release occurs before queue drainage; that recreates the stuck-key/camera race seen in games such as Roblox.
...
- `RootView` owns a 48-point opaque top bar and a solid custom sidebar instead of `NavigationSplitView`, native toolbar material, or blur. This is intentional: Sequoia and Tahoe therefore render the same active colors without Liquid Glass or inactive-sidebar tinting. `WindowChromeConfigurator` makes the native titlebar transparent, preserves the standard traffic lights, and lets only the dedicated top-bar drag handle move the window. Page content remains below the bar.
...
1. All unit/regression tests pass, including compiled equality, exact optimizer/RNG resume, blacklist filtering, cache layout, and long-loop throughput/memory stability.
...
4. Launch the final rebuilt bundle, inspect Record, Library, Models, Training, Run, Diagnostics, Settings, and the menu extra through native UI automation.
