...
- Deployment floor: Apple silicon running macOS Sequoia 15.0. The source and opaque custom chrome support Sequoia 15, Tahoe 26, and macOS 27. Hotkeys use native registration when Carbon is importable and compile an AppKit fallback otherwise, avoiding a hard future-SDK dependency without adding Accessibility to today's recording-only workflow. The 2026-07-15 baseline was built with Swift 6.3.3/Xcode 26.6/SDK 26.5 and exercised on macOS 27 beta. Also compile with Xcode 27 before a public macOS 27 release whenever that toolchain is installed. MLX Swift is pinned exactly to stable 0.31.3.
...
- Crystal V4 and Crystal V4 Fine-tuned + glass are user-protected profiles: autosave pruning, version deletion, learning reset, and profile deletion must leave compatible artifacts intact. Protection becomes sticky when either is saved, so later renames remain safe. A model-schema migration is the one exception: incompatible tensor shapes must be removed for every profile, while the protected profile record and all source recordings remain intact.
...
Model contract schema 4 is the temporal, normalized spatial Policy v4 input and intentionally cannot load older weights. Training-data schema 6 adds exact preceding-perception indices and sub-tick control pulses. On first launch, `WorkspaceStore.removeObsoleteModelArtifacts` preserves every profile and source recording, removes incompatible Versions/Checkpoint directories for every profile (including protected names), maps old architecture capacity to the new Small/Balanced/Large presets, caps history at 32, caps the migrated learning rate at 0.0003, and clears derived caches. Never broaden this migration to source recordings.
...
- AdamW uses a persisted 500-step linear warmup followed by inverse-square-root decay, and global gradient norm is clipped to 1. Exact resume therefore restores the same scheduled update.
...
- Fixed-shape batch temporaries live inside an autorelease pool. MLX cache is bounded to 6% of physical memory with a 2 GiB ceiling; the MLX allocation limit retains at least 15% and normally at least 2 GiB for macOS. Profile validation uses a conservative parameters/optimizer/activation working-set estimate before training starts.
...
- Whole recordings are held out where possible. A segment cannot enter validation if doing so removes the only training example of any binary output. Evaluation uses one fixed class-aware subset (rare positives first, then transitions, active deltas, and even timeline coverage), not the first contiguous batches.
- The latest exact checkpoint and lowest-validation-loss weights are separate. Epoch-end validation updates `best.weights.safetensors`; completed training activates a weights-only Best Brain while the latest optimizer checkpoint remains the continuation source. Explicitly activating a weights-only brain clears a stale checkpoint so later training safely fine-tunes that brain with a fresh optimizer; the selected weights and validation score are first captured as the baseline so a worse first fine-tuning epoch cannot replace them.
...
- Timing, architecture, channels, cursor visibility, and Auto mouse mode come from the immutable selected version rather than mutable editor/Record-tab fields. Startup has a revision token, so Stop during weight loading cannot leave an orphan runtime.
- Runtime feeds the exact previous successfully preprocessed perception into Policy v4. If a published prediction becomes older than `max(350 ms, 3 / Perception FPS)`, the watchdog stops the runtime and releases every held output.
