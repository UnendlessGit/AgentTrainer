...
Policy v4 concatenates the current dense Y or Y/Cb/Cr planes, the signed `current - previous` planes, and normalized X/Y channels before the first convolution. Four default GroupNorm/SiLU stages use a stride-four stem and reach a 63-pixel receptive field with a cumulative stride of 32. New architectures score the final grid with multiple learned spatial-attention keypoints. Each keypoint pools local features plus exact X/Y, while global mean and maximum features preserve whole-scene context. This retains layout without making the first dense projection proportional to every grid cell; learned parameter count is therefore nearly resolution-independent. `Flattened Grid (Legacy)` remains an explicit architecture value, and a missing optional pooling field decodes to it so existing weights retain their exact tensor contract. The exact output geometry and both parameter formulas are shared by `AgentPolicy`, `CNNGeometry`, and `ModelSizing`. Deterministic coordinate buffers are deliberately underscore-prefixed so MLX module reflection does not treat them as trainable/saved tensors or allocate AdamW moments; regression tests compare complete reflected GRU and LSTM parameter trees with `ModelSizing`.
...
- New profiles use persisted one-epoch linear warmup, bounded to 10–500 steps, followed by configurable bounded cosine restarts. Epoch-end held-out loss—or epoch-average training loss when honest validation is unavailable—drives a 0.2%-tolerance plateau monitor. After the configured patience it halves the cosine envelope without crossing the configured global learning-rate floor. Step, schedule, cycle length, envelope scale, best monitor value, patience counter, and reductions are checkpointed. Profiles/checkpoints without a schedule field intentionally retain historical inverse-square-root decay; selecting the adaptive schedule changes checkpoint identity, preserves compatible runnable weights, and begins a fresh optimizer sequence. Global gradient norm is clipped to 1.
...
- Binary controls use training-split positive weights up to 1024×, transition weights, configurable focal weights, and a zero loss mask for unseen/blocked outputs. Transition comparison uses a separately gathered real preceding action row even when `historyLength == 0`; segment starts use released zero state. Active relative-mouse and scroll targets use weighted Smooth L1; absolute cursor uses Smooth L1. This prevents easy idle examples from dominating while retaining robust continuous losses. The independent 50% history-branch mask is not inverted dropout: kept rows remain at inference magnitude.
- Whole recordings are held out where possible. Selection keeps the requested recording count but greedily targets the requested **sample** fraction, rewards new control coverage, and cannot remove the only training example of an enabled, permitted binary output. Blocked/disabled controls cannot erase validation availability or consume its representative budget. With one recording, the temporal boundary is extended until validation's full recurrent history and both perception indices are disjoint from training; irregular/static delivery is checked through actual observation indices rather than an FPS estimate. If no honest held-out tail remains, all samples train and validation is disabled. The fixed class-aware evaluation budget scales with held-out size and recording diversity up to 8,192 rows; rare positives, transitions, active deltas, every segment's midpoint/endpoints, and even timeline coverage are prioritized. Reports persist sample count, aggregate and per-head binary confusion metrics, absolute/active continuous MAE, and idle continuous false-action rate.
- The latest exact checkpoint and best validation-safe weights are separate. Epoch-end validation updates `best.weights.safetensors` only when aggregate held-out loss improves without a well-supported F1 collapse greater than 0.10 or a false-positive-rate spike greater than 0.03 in the buttons, keyboard, or modifiers head. Completed training activates a weights-only Best Brain while the latest optimizer checkpoint remains the continuation source. Explicitly activating a weights-only brain clears a stale checkpoint so later training safely fine-tunes that brain with a fresh optimizer. Before the first update, those selected weights are evaluated on the current run's held-out set and captured as the comparable baseline; a saved score from different recordings, targets, split, or loss semantics is never compared directly. The validation-strategy version is checkpointed independently, so a contract upgrade recalibrates the comparison while retaining compatible optimizer state; if the current split has no honest validation rows, stale validation/best metadata is cleared.
